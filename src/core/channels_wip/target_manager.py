...
class TargetManager:
    ...
    def update(self, tracks, predicted_center, frame_shape):
        ...
        if current_healthy:
            if (
                score_gap < max(self.hard_switch_min_gain, 1.45)
                and dist_best_to_anchor >= dist_current_to_anchor * 0.88
                and center_gap < max(self.owner_switch_min_gap_px * 1.35, self.predicted_dist_px * 0.50)
                and not same_raw
            ):
                self.pending_id = None
                self.pending_count = 0
        ...
        margin = self.switch_margin * (0.35 if current_degraded else 1.45)
        ratio_ok = best_score > (current_score * (1.02 if current_degraded else 1.18))
        dwell_ok = self.lock_age >= (3 if current_degraded else max(self.min_hold_frames + 1, self.switch_dwell))
        cooldown_ok = (self.frame_id - self.last_switch_frame) >= (3 if current_degraded else max(self.switch_cooldown, 10))
        geometry_separation_ok = center_gap >= (10.0 if current_degraded else self.owner_switch_min_gap_px * 1.15)
