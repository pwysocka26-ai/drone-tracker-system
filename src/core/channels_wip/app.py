...
from core.lock_pipeline import LockPipeline, STATE_LOCKED, STATE_REFINE, STATE_ACQUIRE
...
def run_app(config):
    ...
    runtime_lock_phase = "UNLOCKED"
    runtime_lock_measure_count = 0
    runtime_lock_break_count = 0
    runtime_lock_active = False
    lock_warmup_frames = max(2, int(narrow_cfg.get("lock_warmup_frames", 3)))
    lock_break_confirm_frames = max(2, int(narrow_cfg.get("lock_break_confirm_frames", 3)))
    ...
    def reset_runtime_lock_state():
        nonlocal runtime_lock_phase, runtime_lock_measure_count, runtime_lock_break_count, runtime_lock_active
        runtime_lock_phase = "UNLOCKED"
        runtime_lock_measure_count = 0
        runtime_lock_break_count = 0
        runtime_lock_active = False
    ...
    try:
        while True:
            ...
            active_track = pipeline_soft_track if pipeline_soft_track is not None else selected_track
            ...
            if smooth_center is not None:
                ...
                geom_for_lock = float(lock_info.get('geometry_score')) if lock_info.get('geometry_score') is not None else None
                stable_geom = (geom_for_lock is None) or (geom_for_lock >= 0.55)
                measured_ok = bool(measured_center_lock) and (not edge_limit_active) and stable_geom
                owner_present = effective_track is not None
                if not owner_present:
                    ...
                    runtime_lock_break_count = 0
                    center_lock = True
                    if runtime_lock_phase in ("UNLOCKED", "RECOVERING"):
                        runtime_lock_phase = "WARMUP"
                        runtime_lock_measure_count = 1
                    elif runtime_lock_phase == "WARMUP":
                        runtime_lock_measure_count += 1
                        if runtime_lock_measure_count >= lock_warmup_frames:
                            runtime_lock_phase = "LOCKED"
                    else:
                        runtime_lock_phase = "LOCKED"
                        runtime_lock_measure_count = lock_warmup_frames
                    runtime_lock_active = runtime_lock_phase in ("WARMUP", "LOCKED")
                else:
                    center_lock = False
                    if runtime_lock_phase in ("WARMUP", "LOCKED"):
                        runtime_lock_break_count += 1
                        if runtime_lock_break_count >= lock_break_confirm_frames:
                            runtime_lock_phase = "RECOVERING"
                            runtime_lock_measure_count = 0
                            runtime_lock_active = False
                        else:
                            runtime_lock_active = True
                    else:
                        runtime_lock_phase = "RECOVERING" if owner_present else "UNLOCKED"
                        runtime_lock_measure_count = 0
                        runtime_lock_active = False
                ...
                cv2.putText(narrow_output, f'LOCK PHASE: {runtime_lock_phase}', (20, 328), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 2)
            ...
            current_lock_state = 'TRACKING' if runtime_lock_active else 'REACQUIRE'
            ...
            if telemetry_enabled and telemetry is not None:
                ...
                telemetry.log_frame(
                    ...
                    lock_state=str('TRACKING' if runtime_lock_active else 'REACQUIRE'),
